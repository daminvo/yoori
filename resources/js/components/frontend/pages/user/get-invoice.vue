            ...
            <div class="container" v-for="(order,index) in orders" :key="index">
                <div class="invoice_border mt-2">
                    <div class="page-title">
                        ...
                        <p v-if="authUser">{{lang.a_copy_summary_has_been_sent_to}} <a :href="'mailto'+authUser.email">{{ authUser.email }}</a></p>
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                <tr v-for="(order_detail,index) in order.order_details" :key="'order'+index">
                                    ...
                                    <td class="text-start">{{ priceFormat(order_detail.price * order_detail.quantity) }}</td>
                                    <td class="text-start">{{ priceFormat(order_detail.tax) }}</td>
                                    <td class="text-start">{{ priceFormat(order_detail.discount * order_detail.quantity) }}</td>
                                    <td class="text-start" v-if="settings.coupon_system == 1">{{ priceFormat(order_detail.coupon_discount.discount) }}</td>
                                    <td class="text-start">{{ priceFormat(order_detail.shipping_cost.total_cost)  }}</td>
                                    <td class="text-start">{{ priceFormat(((order_detail.price * order_detail.quantity) + order_detail.tax + order_detail.shipping_cost.total_cost ) - ((order_detail.discount * order_detail.quantity) + order_detail.coupon_discount.discount)) }}</td>
                                </tr>
                                </tbody>
                            </table>
                        </div>
                                    ...
                                    <div class="order-summary">
                                        <div class="sg-card">
                                            <ul class="global-list">
                                                ...
                                                <li>{{lang.discount_offer }}<span>{{ priceFormat(order.discount) }}</span></li>
                                                <li v-if="settings.coupon_system == 1">{{lang.coupon_discount}} <span>{{ priceFormat(order.coupon_discount) }}</span></li>
                                                <li>{{lang.tax}} <span>{{ priceFormat(order.total_tax) }}</span></li>
                                                <li>{{lang.shipping_cost}} <span>{{ priceFormat(order.shipping_cost) }}</span></li>
                                            </ul>
                                            <div class="order-total mb-0">
                                                <p>Total <span>{{ priceFormat(order.total_payable) }}</span></p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
