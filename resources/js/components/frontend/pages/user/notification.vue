            ...
            <div class="sg-table sg-notification">
              ...
              <table v-if="is_shimmer" class="table">
                ...
                <tr v-for="(notification, i) in notifications.data" :key="i"
                    v-if="notification.is_deleted != 1">
                  <th scope="row">{{ ++i }}</th>
                  ...
                  <td>
                    <div class="add-to-cart mb-2 mt-2">
                      <a v-if="notification.status == 'unseen'" href="javascript:void(0)"
                         @click="seen(notification.id,i)" class="btn ">{{ lang.seen }}</a>
                      <a href="javascript:void(0)" @click="remove(notification.id,i)" class="btn">{{ lang.remove }}</a>
                    </div>
                  </td>
                </tr>
                </tbody>
    ...
    remove(id, i) {
      ...
      axios.get(url).then((response) => {
        if (response.data.error) {
          ...
          this.notifications.data[i].is_deleted = 1;
