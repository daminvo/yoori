                                ...
                                <div class="category-list">
                                    <ul class="global-list" v-if="category.child_categories && category.child_categories.length > 0">
                                        <li v-for="(child_category,child_index) in category.child_categories" :key="child_index" :class="{ 'has-baby' : child_category.categories.length > 0 }">
                                            ...
                                            <ul v-if="child_category.categories.length > 0" v-for="(category,index) in child_category.categories"  :key="'child_category'+index">
                                                <li><a :href="getUrl('category/'+category.slug)" @click.prevent="routerNavigator('product.by.category',category.slug)">{{ category.title }}</a></li>
                                            </ul>
                                        </li>
                                    </ul>
                                    <ul v-else>
