          ...
          <a :href="'product/'+product.slug" @click.prevent="routerNavigator('product.details',product.slug)">
            <div class="product-thumb">
                            <span class="base" v-if="product.special_discount_check > 0">{{
                                product.special_discount_type == 'flat' ? priceFormat(product.special_discount_check) + ' OFF' : product.special_discount_check + '% OFF'
                              }} </span>
              <span class="base reword-badge" v-if="addons.includes('reward') && product.reward > 0">{{
                  lang.reward_point
                }}: {{ product.reward }}</span>
              <img v-lazy="product.image_190x230" :alt="product.product_name" class="img-fluid">
            </div>
          </a>
          <div class="product-info">
                                    <span class="price"><del
                                        v-if="product.special_discount_check > 0">{{ priceFormat(product.price) }}</del>
            ...
            <div class="icons">
              <ul class="global-list">
                <li v-if="product.minimum_order_quantity <= product.current_stock"><a
                    href="javaScript:void(0)" @click="cartBtn(product,index)"><span
                    class="mdi mdi-name mdi-shopping-outline"></span></a>
                </li>
                <span v-if="authUser">
                  <li v-if="checkWishlist(product) || product.user_wishlist"><a href="javaScript:void(0)" @click="removeWishlist(product.id)"><span class="mdi mdi-name mdi-heart"></span></a></li>
                  <li v-else><a href="javaScript:void(0)" :class="{ 'disable_btn' :  is_disabled == true }" @click="addToWishlist(product.id)"><span class="mdi mdi-name mdi-heart-outline"></span></a></li>
                </span>
                <li v-if="compareCheck(product)"><a class="active_compare" :class="{ 'disable_btn' :  is_disabled == true }" @click="removeCompare(product.id)" href="javaScript:void(0)"><span class="mdi mdi-name mdi-scale-balance"></span></a></li>
                <li v-else><a @click="addToCompare(product)" :class="{ 'disable_btn' :  is_disabled == true }" href="javaScript:void(0)"><span class="mdi mdi-name mdi-scale-balance"></span></a></li>
                <li><a href="javaScript:void(0)" @click="productFetch(product.slug)"><span class="mdi mdi-name mdi-magnify"></span></a></li>
              </ul>
            </div>
          </div>
        </div>
