                    ...
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tr>
                                ...
                                <th scope="col" class="text-start">{{ lang.price }}</th>
                                <th scope="col" class="text-start">{{ lang.sub_total }}</th>
                                <th scope="col" class="text-start">{{ lang.tax }}</th>
                                <th scope="col" class="text-start">{{ lang.discount }}</th>
                                <th scope="col" class="text-start">{{ lang.coupon_discount }}</th>
                                <th scope="col" class="text-start">{{ lang.shipping_cost }}</th>
                                <th scope="col" class="text-start">{{ lang.total_amount }}</th>
                            ...
                            <tr v-for="(order_detail,index) in order.order_details" :key="'order'+index">
                                ...
                                <td class="text-start">{{ order_detail.quantity }}</td>
                                <td class="text-start">{{ priceFormat(order_detail.price) }}</td>
                                <td class="text-start">{{ priceFormat(order_detail.price * order_detail.quantity) }}
                                </td>
                                <td class="text-start">{{ priceFormat(order_detail.tax) }}</td>
                                <td class="text-start">{{ priceFormat(order_detail.discount) }}</td>
                                <td class="text-start" v-if="settings.coupon_system == 1">
                                    {{ priceFormat(order_detail.coupon_discount.discount) }}
                                </td>
                                <td class="text-start">{{ priceFormat(order_detail.shipping_cost.total_cost) }}</td>
                                <td class="text-start">
                                    {{
                                        priceFormat(((order_detail.price * order_detail.quantity) + order_detail.tax + order_detail.shipping_cost.total_cost) - (order_detail.discount + order_detail.coupon_discount.discount))
                                    }}
                                </td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <ul class="global-list">
                                            ...
                                            <li class="text-capitalize">{{ order.payment_type }}
                                            </li>
                                        </ul>
                                    </td>
                                </tr>
                                ...
                                <div class="order-summary">
                                    <div class="sg-card">
                                        <ul class="global-list">
                                            ...
                                            <li>{{ lang.discount_offer }} <span>{{ priceFormat(order.discount) }}</span>
                                            </li>
                                            <li>{{ lang.coupon_discount }} <span>{{
                                                    priceFormat(order.coupon_discount)
                                                }}</span></li>
                                            <li>{{ lang.tax }} <span>{{ priceFormat(order.total_tax) }}</span></li>
                                            <li>{{ lang.shipping_tax }} <span>{{
                                ...
                                <form @submit.prevent="refundSubmit()">
                                    <div class="modal-body text-left">
                                        <div class="form-group">
                                            ...
                                            <input type="hidden" v-model="form.order_detail_id" class="form-control"
                                                   disabled>
                                            ...
                                            <span class="validation_error"
                                                  v-if="errors.order_detail_id">{{ errors.order_detail_id[0] }}</span>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    <label for="order_id">{{ lang.total_paid }}</label>
                                                    <input type="text" v-model="form.total_amount" class="form-control"
                                                           :class="{ 'error_border' : errors.total_amount }" disabled
                                                           id="order_id">
                                                </div>
                                                <span class="validation_error"
                                                      v-if="errors.total_amount">{{ errors.total_amount[0] }}</span>
                                            </div>
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    <label for="shipping_cost">{{ lang.shipping_cost }}</label>
                                                    <input type="text" v-model="form.shipping_cost" class="form-control"
                                                           :class="{ 'error_border' : errors.shipping_cost }" disabled
                                                           id="shipping_cost">
                                                </div>
                                                <span class="validation_error"
                                                      v-if="errors.shipping_cost">{{ errors.shipping_cost[0] }}</span>
                                            </div>
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    <label for="refundable_amount">{{ lang.refundable_amount }}</label>
                                                    <input type="text" v-model="form.refundable_amount"
                                                           class="form-control"
                                                           :class="{ 'error_border' : errors.refundable_amount }"
                                                           disabled id="refundable_amount">
                                                </div>
                                            </div>
                                        ...
                                        <div class="form-group">
                                            <label for="refund_reason"
                                                   class="col-form-label">{{ lang.refund_reason }}</label>
                                            <textarea class="form-control" v-model="form.refund_reason"
                                                      :class="{ 'error_border' : errors.refund_reason }"
                                                      id="refund_reason"></textarea>
                                        </div>
                                        <span class="validation_error"
                                              v-if="errors.refund_reason">{{ errors.refund_reason[0] }}</span>

                                        <div class="modal-footer">
                                            <button type="submit" class="btn btn-primary" v-show="!loading">
                                                {{ lang.send_request }}
                                            </button>
                                            <loading_button v-show="loading"
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body text-left">
                                    <div class="form-group">
                                        ...
                                        <input type="text" v-model="product_name_show" class="form-control" disabled
                                               id="product">
                                    </div>
                                    <div class="row">
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                <label for="total_paid" class="col-form-label">{{
                                                        lang.total_paid
                                                    }}</label>
                                                <input type="text" v-model="total_amount_show" class="form-control"
                                                       disabled
                                                       id="total_paid">
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                <label for="shipping_cost_show"
                                                       class="col-form-label">{{ lang.shipping_cost }}</label>
                                                <input type="text" v-model="shipping_cost_show" class="form-control"
                                                       disabled
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                <label for="refundable_amount_show"
                                                       class="col-form-label">{{ lang.refundable_amount }}</label>
                                                <input type="text" v-model="refundable_amount_show" class="form-control"
                                                       disabled id="refundable_amount_show">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label for="refund_reason_show" class="col-form-label">{{
                                                lang.refund_reason
                                            }}</label>
                                    ...
                                    <div class="form-group" v-if="reject_reason_show">
                                        <label for="reject_reason" class="col-form-label">{{
                                                lang.reject_reason
                                            }}</label>
                                        <textarea class="form-control" v-model="reject_reason_show" disabled
