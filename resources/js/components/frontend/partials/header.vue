<template>
  <header class="sg-header home-4" :class="{ 'sidebar-active': show_sm_category,'mobile-menu-active' : show_sm_home }">
    <div class="sg-topbar">
      <div class="container">
        <div class="topbar-content">
                ...
                <div class="dropdown">
                  ...
                  <ul @click.stop class="dropdown-menu" :class="{ show: language_dropdown }" aria-labelledby="">
                    <li v-for="(language, index) in languages" :key="index">
                      <a class="dropdown-item" @click="changeLanguage(language.locale)" href="javascript:void(0)"
                      ><img v-lazy="language.flag_image" alt="flag" class="img-fluid"/>{{ language.name }}</a
                      >
                    </li>
                  </ul>
                </div>
              ...
              <li v-if="settings.currency_switcher != 0">
                <div class="dropdown">
                  ...
                  <ul @click.stop class="dropdown-menu" :class="{ show: currency_dropdown }" aria-labelledby="">
                    ...
                    ><a class="dropdown-item" @click="changeCurrency(currency)"
                        href="javascript:void(0)">{{ currency.name }}</a></li
                    >
                  </ul>
                </div>
              </li>
              <li v-if="settings.topbar_play_store_link == 1">
                <a :href="settings.play_store_link" target="_blank"><span class="mdi mdi-name mdi-google-play"></span> <small>{{ lang.play_store }}</small></a>
              </li>
              <li v-if="settings.topbar_app_store_link == 1">
                <a :href="settings.apple_store_link" target="_blank"><span class="mdi mdi-name mdi-apple"></span> <small>{{ lang.app_store }}</small></a>
              </li>
          ...
          <div class="right-content new-list">
            <ul class="global-list">
              ...
              <li>
                <router-link :to="{ name: 'compare.list' }">{{ lang.compare }} <span
                    v-if="compareList > 0">({{ compareList }})</span></router-link>
              </li>
              <li>
            ...
            <div class="search-form">
              <div class="btn-group w-100">
                ...
                <a class="search_btn cursor_pointer"
                  ...
                  <img :src="defaultAssets.search_image" alt="Search" class="img-fluid"/>
                </a>
            ...
            <ul class="global-list user-shop-option">
              <li>
                <router-link :to="{ name: 'wishlist' }">
                  <div class="icon"
                  ><img alt="Compare Icon" class="img-fluid" :src="defaultAssets.wishlist_svg"/>
                    <!---->
                  </div>
              ...
              <li class="sg-dropdown cart">
                <router-link :to="{ name: 'cart' }" class="">
                  <div class="icon"
                  ><img alt="Svg Icon" class="img-fluid" :src="defaultAssets.bag_image"/>
                      ...
                      <div class="sg-product">
                        ...
                        <div class="product-thumb">
                          <router-link :to="{ name: 'product.details', params: { slug: cart.product_slug } }">
                            <img v-lazy="cart.image_40x40" :alt="cart.product_name" class="img-fluid"
                            /></router-link>
                        </div>
              ...
              <li class="compare-icon">
                <router-link :to="{ name: 'compare.list' }">
                  <div class="icon"><img :src="defaultAssets.compare_svg" alt="List"/></div>
              ...
              <li class="d-flex">
                <a :href="'tel:' + settings.header_contact_phone" class="live-chat">
                  <div class="icon"
                  ><span><img alt="Phone Icon" class="img-fluid" :src="defaultAssets.phone_image"/></span
                  ></div>
                </a>
          ...
          <div class="sg-categorie-menu categorie-lg align-self-lg-center">
            <div class="top-content">
              <button class="sg-toggle" @click="toggleCategory">
                <!-- <img :src="defaultAssets.list" alt="category"> -->
            ...
            <div class="categorie-menu" id="categorie-menu" v-if="defaultCategoryShow || $route.name == 'home'">
                ...
                <ul class="global-list" v-if="lengthCounter(visibleCategory)>0">
                    ...
                    <div v-if="category.child_categories.length > 0" class="sg-dropdown-menu">
                      <div class="row">
                        <div :class="[category.latest_product ? 'col-lg-9' : 'col-lg-12']">
                          <div class="row">
                            <div class="col-lg-4" v-for="(child_category, child_index) in category.child_categories"
                                 :key="child_index">
                              <h3><a href="#">{{ child_category.title }}</a></h3>
                  ...
                  <li>
                    <router-link :to="{ name: 'categories' }">
                      <strong>{{ lang.view_all_categories }}</strong>
                      <span class="mdi mdi-name mdi-chevron-right"></span
                      ></router-link>
                  </li>
                </ul>
                <ul class="global-list" v-else-if="sidebar_category">
                  <li v-for="(seller,index) in 12" :key="index">
                    <a href="javascript:void(0)"  class="shimmer">
                      <span><shimmer :height="25"></shimmer></span>
                      <shimmer :height="25"></shimmer>
                    </a>
                  </li>
                </ul>
              </div>
                ...
                <div class="collapse navbar-collapse" id="navbarNav" :class="{ show: menu }">
                  <ul class="navbar-nav">
                    <li class="nav-item" v-for="(menu, i) in headerMenu" :key="'menu' + i"
                      ...
                      <a v-if="urlCheck(menu.url)" :href="menu.url"  @click="subMenuToggle" @mouseover="subMenuToggle"
                      >{{ menu.label }}<span v-if="Object.keys(menu).length > 2"
    ...
    <div class="sg-categorie-menu categorie-sm">
      ...
      <div class="categorie-menu-content">
        ...
        <ul @click.stop class="global-list">
          <li class="active" @click="show_sm_category = false">
            <router-link :to="{ name: 'campaigns' }"><span class="mdi mdi-view-dashboard-outline mdi-18px"></span>{{
                lang.all_offers
              }}
            </router-link>
          </li>
          ...
          <li class="active" v-for="(category, index) in visibleCategory" :key="index"
            ...
            <router-link :to="{ name: 'product.by.category', params: { slug: category.slug } }">
              <span :class="[category.icon]" class="mdi-18px" v-if="category.icon"></span>
              <span v-else class="mdi mdi-view-dashboard-outline mdi-18px"></span>{{ category.title }}
            </router-link>
            ...
            <ul class="sg-dropdown-menu"
              ...
              <li class="active" v-for="(child_category, index) in category.child_categories" :key="index"
                ...
                <router-link :to="{ name: 'product.by.category', params: { slug: child_category.slug } }">
                  <span :class="[child_category.icon]" class="mdi-18px" v-if="child_category.icon"></span>
                  <span v-else class="mdi mdi-view-dashboard-outline mdi-18px"></span>{{ child_category.title }}
                </router-link>
                <ul class="sg-dropdown-menu"
                    :class="{ 'd-block': mobile_children_id == child_category.id, 'd-none': mobile_children_id != child_category.id }" v-if="child_category.categories.length > 0">
                  <li v-for="(children_category, cat_index) in child_category.categories" :key="cat_index">
                    <router-link :to="{ name: 'product.by.category', params: { slug: children_category.slug } }">
                      <span :class="[children_category.icon]" class="mdi-18px" v-if="children_category.icon"></span>
                      <span v-else class="mdi mdi-view-dashboard-outline mdi-18px"></span>{{ children_category.title }}
                    </router-link>
                  </li>
                </ul>
          ...
          <li @click="show_sm_category = false" class="sm-last-item">
            <router-link :to="{ name: 'categories' }"
            ><strong>{{ lang.view_all_categories }}</strong> <span class="mdi mdi-name mdi-chevron-right"></span
            ></router-link>
          </li>
        </ul>
      </div>
          ...
          <li class="nav-item" v-for="(menu, i) in headerMenu" :key="'menu' + i"
            ...
            <router-link v-else :to="menu.url" :class="{ active: menu.url == $route.fullPath }">
              {{ menu.label }}<span v-if="Object.keys(menu).length > 2"
                                    class="icon mdi mdi-name mdi-chevron-down"></span
            ></router-link>
            <ul class="sg-dropdown-menu" :class="{ 'd-block': home_child_id == menu.id, 'd-none': home_child_id != menu.id }"
                v-if="Object.keys(menu).length > 2">
              <li v-for="(sub_menu, j) in menu" :key="'sub_menu' + j" v-if="sub_menu.url">
                <a v-if="urlCheck(sub_menu.url)" :href="sub_menu.url">{{ sub_menu.label }}</a>
                <router-link v-else :to="sub_menu.url">{{ sub_menu.label }}</router-link>
          ...
          <ul class="global-list">
            <li>
              <a href="javascript:void(0)" @click.stop="homeMenu" class="sg-toggle">
                <img :src="defaultAssets.list" alt="menu"/>
              </a>
            </li>
            <li>
              <router-link :to="{ name: 'home' }" aria-current="page">
                <img :src="defaultAssets.home" alt="Home"/>
              </router-link>
            </li>
            <li>
              <a href="javascript:void(0)" @click.stop="categoryMenu" class="sg-toggle">
                <img :src="defaultAssets.menu_svg" alt="List"/>
              </a>
            </li>
            <li>
              <router-link :to="{ name: 'cart' }" class="c-toggle">
                <img :src="defaultAssets.shopping_bag" alt="shopping-bag"/>
              </router-link>
            </li>
            <li>
              <div v-if="authUser">
                <router-link v-if="authUser.user_type == 'customer'" :to="{ name: 'dashboard' }">
                  <img :src="defaultAssets.user" alt="User"/>
                </router-link>
                <a class="d-block" :href="getUrl('seller/dashboard')" v-else-if="authUser.user_type == 'seller'"
                ><img :src="defaultAssets.user" alt="User"
                /></a>
                <a class="d-block" :href="getUrl('admin/dashboard')"
...
export default {
  name: "headNav",
  components: {About, detailsView, shimmer},
  data() {
    return {
  ...
  methods: {
    ...
    changeCurrency(currency) {
      let url = this.getUrl("change/currency/" + currency.id);
