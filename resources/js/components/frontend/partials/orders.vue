        ...
        <table class="table">
            ...
            <tr v-for="(orderDetails,i) in orders" :key="i" v-if="orderDetails.is_deleted == 0">
                <th scope="row">
                    <div class="product">
                        <div class="text pl-1">
                            <p class="text-ellipse" v-if="orderDetails.product_name">{{ orderDetails.product_name }}</p>
                ...
                <td  class="text-capitalize">
                    <div  :class="orderDetails.delivery_status == 'pending' ? 'pending' : (orderDetails.delivery_status == 'delivered' ? 'complete' : (orderDetails.delivery_status == 'canceled' ? 'cancel' : 'delivery-info'))">
                    <span>
                ...
                <td>
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu text-capitalize"  :class="orderDetails.order_id == order_dropdown ? 'show' : ''">
                            ...
                            <li>
                                <router-link v-if="orderDetails.payment_status == 'unpaid' && orderDetails.payment_type != 'cash_on_delivery' && orderDetails.delivery_status != 'cancelled' && orderDetails.delivery_status != 'offline_method'" :to="{name: 'payment' , params : { code : orderDetails.code } }">{{lang.pay_now}}</router-link>
                            </li>
                            <li>
                                <router-link :to="{name: 'get.invoice',params:{orderCode:orderDetails.code}}">{{lang.view}}</router-link>
                            </li>
                            <li>
                                <a href="javascript:void(0)" @click="download(orderDetails.order_id, orderDetails.code)">{{lang.download}}</a>
                            </li>
                            <li v-if="orderDetails.product_file_id && orderDetails.payment_status == 'paid'">
                                <a v-for="(url,index) in orderUrls" :key="index" v-if="index == orderDetails.id" :href="url">{{lang.download_file}}</a>
                            </li>
                            <li v-if="orderDetails.delivery_status == 'delivered' || orderDetails.delivery_status == 'canceled'">
                                <a href="javascript:void(0)" @click="removeOrder(orderDetails.order,i)">{{lang.delete}}</a>
                            </li>
                        </ul>
                    </div>
                </td>
...
export default {
    ...
    methods: {
        removeOrder(order_id,i) {
            let url = this.getUrl('user/remove-order/' + order_id.id);
            this.$Progress.start();
            axios.get(url).then((response) => {
                if (response.data.error) {
                    ...
                    this.page--
                    this.orders[i].order.is_deleted = 1
                }
                this.order_dropdown =""
        ...
        cancelOrder(order_id,i) {
            let url = this.getUrl('user/cancel-order/' + order_id.id);
            if (confirm("Are you sure?"))
            {
                ...
                axios.get(url).then((response) => {
                    if (response.data.error) {
                        ...
                        this.orders[i].order.delivery_status = 'canceled'
                        this.orders[i].order.payment_status = this.orders[i].order.payment_status == 'unpaid' ? 'unpaid' : this.lang.refunded_to_wallet
