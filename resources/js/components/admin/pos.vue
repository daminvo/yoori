<template>
  <div class="pos-section pos-section--modal mb-5">
                ...
                <a class="custom--card-style" href="javascript:void(0)"
                  ...
                  <div class="card pos-card">
                    ...
                    <div class="thumb mb_39">
                      <img v-if="stockProduct.has_variant == 1" :src="stockProduct.stock_image"
                           class="img-fluid">
                      <img v-else :src="stockProduct.product_image"
                           class="img-fluid">
                    ...
                    <div class="sg-price justify-content-center"
                         v-if="stockProduct.has_variant == 1">
                                                <span class="price"><del
                                                    v-if="stockProduct.special_discount > 0">{{
                                                    priceFormat(stockProduct.product_price)
                                                  }}</del>
                                                    <span v-if="stockProduct.special_discount > 0">
                                                            {{ priceFormat(stockProduct.stock_discount_price) }}
                                                        </span>
                                                    <span v-else>{{ priceFormat(stockProduct.stock_price) }}</span>
                                                </span>
                      ...
                      <span class="variant_badge"
                            v-if="stockProduct.name">{{ stockProduct.name }}</span>
                    </div>
                    <div class="sg-price justify-content-center" v-else>
                                                <span class="price"><del
                                                    v-if="stockProduct.special_discount > 0">{{
                                                    priceFormat(stockProduct.product_price)
                                                  }}</del>
                                                    ...
                                                    <span v-else>{{ priceFormat(stockProduct.product_price) }}</span>
                                                </span>
                      <span v-if=" stockProduct.current_stock >0"
                      ...
                      <span v-else class="stock-badge"
                            :class="{ 'red_badge' : stockProduct.current_stock == 0 }">{{ lang.out_of_stock }}</span>
                      <span class="variant_badge"
                            v-if="stockProduct.name">{{ stockProduct.name }}</span>
                    </div>
                  </div>
            ...
            <div class="card">
              ...
              <div class="sub-total">
                <ul class="global-list">
                  ...
                  <li>{{ lang.vat_tax }}<span>{{ priceFormat(orderAmount.taxes) }}</span></li>
                ...
                <div class="order-total">
                  <p>{{ lang.totals }} <span>{{ priceFormat(orderAmount.total) }}</span></p>
                </div>
              </div>
              ...
              <div class="form-group pos_payment_dropdown text-right">
                ...
                <button class="btn btn-outline-info btn-circle" type="button" data-toggle="modal"
                        @click="draftList()" data-target="#draft-list">{{ lang.draft_list }}
                </button>
    ...
    <draft :draft="draft" :loading_image="loading_image" :lang="lang" :loading_for_draft=loading_for_draft
           :cartProducts="cartProducts" @draftOrder="getDraftOrder"/>
...
export default {
  ...
  methods: {
    ...
    cartProduct(stock, search) {
      if (!this.cartProducts.some(el => el.id === stock.id)) {
        if (this.stockFind(stock) > 0) {
          ...
          let price = this.priceFind(stock);
          if (search == 'search') {
            let search = 'search'
          }
    ...
    cartPlus(cart) {
      ...
      if (product) {
        let stock = this.stockFind(product);
    ...
    getDraftOrder(trx_id) {
      ...
      axios.post(this.url + '/admin/pos/draft-to-cart', trxId).then((response) => {
        ...
        this.orderAmount.subTotal = response.data.order.sub_total;
        ...
        this.searchingProduct = this.searchingProduct.filter(product => product.product_image);
      }).catch((error) => {
