            ...
            <div class="d-flex justify-content-between">
                ...
                <div class="text-right d-flex">
                    <div class="buttons add-button">
                        <a href="{{ route('admin.add.media') }}" class="btn btn-icon icon-left btn-outline-primary">
                            <i class="bx bx-plus"></i>{{ __('Upload File') }}</a>
                    </div>
                    <div class="buttons add-button setting">
                        <a href="{{ route('storage.setting') }}" class="btn btn-icon icon-left btn-outline-primary" data-toggle="tooltip" data-original-title="{{ __('Storage Configuration') }}">
                                                ...
                                                <label class="imagecheck mb-4">
                                                    <figure class="imagecheck-figure">
                                                        @if($media->type == 'image' && @is_file_exists($media->image_variants['image_190x230'] , $media->storage))
                                                            <img src="{{ get_media($media->image_variants['image_190x230'], $media->storage) }}" alt="{{ $media->name }}"
                                                                 class="imagecheck-image article-image">
                                                        @else
                                                            <img src="{{ static_asset('images/default/default-'.$media->type.'-190x230.png') }}" alt="{{ $media->name }}"
                                                                 class="imagecheck-image article-image">
                                                        @endif
                                                    </figure>
                                                </label>
                                        ...
                                        <div>
                                            <div class="media__content">
                                                ...
                                                <div class="article-details">
                                                    <span class="image-size d-block my-1">{{ formatBytes($media->size) }} | <span class="uppercase">{{ $media->extension }}</span></span>
                                                ...
                                                <div class="center bottom-fixing">
                                                    <div class="d-flex center">
                                                        ...
                                                        @endif
                                                        <a href="javascript:void(0)"  data-text="{{ __('Copied to Clipboard') }}" data-url="{{ get_media($media->original_file, $media->storage) }}" class="dropdown-item {{ (hasPermission('media_delete') || Sentinel::getUser()->user_type == 'seller') ? 'ml-2' : '' }} copy-to-clipboard btn btn-outline-info btn-circle">
                                                            <i class='bx bx-copy'></i>
                                                        </a>
                                                    </div>
                                                    <div class="center mt-1">
                                                        <a href="{{ get_media($media->original_file, $media->storage) }}" target="_blank" download="{{ $media->name }}.{{ $media->extension }}" class="dropdown-item btn btn-outline-success btn-circle">
