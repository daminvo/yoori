...
@section('title')
    {{ __('Campaign Requests') }}
@endsection
@section('marketing_active')
    active
@endsection
@section('campaign_request')
    active
@endsection
    ...
    <section class="section">
        <div class="section-body">
            <div class="d-flex justify-content-between">
                <div class="d-block">
                    <h2 class="section-title">{{ __('Campaign Requests') }}</h2>
                    <p class="section-lead">
                        {{ __('You have total') . ' ' . $products->total() . ' ' . __('requests') }}
                    </p>
                </div>
            </div>
        </div>
                            ...
                            <table class="table table-striped table-md">
                                ...
                                @foreach ($products as $key => $value)
                                    <tr id="row_{{ $value->id }}" class="table-data-row">
                                        ...
                                        <td>
                                            {{ $value->campaign->getTranslation('title', \App::getLocale()) }}
                                        </td>

                                        <td>{{ $value->user->first_name }} {{ $value->user->first_name }}</td>
                                        <td>{{ $value->user->sellerProfile->shop_name }} </td>
                                        <td> {{ @$value->product->getTranslation('name', \App::getLocale()) }} </td>
