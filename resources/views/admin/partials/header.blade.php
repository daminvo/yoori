...
<nav class="navbar navbar-expand-lg main-navbar">
    ...
    <ul class="navbar-nav navbar-right">
        @if(addon_is_activated('pos_system') && authUser()->user_type != 'seller')
            <li>
                <a  href="{{ route('admin.pos.system') }}" target="_blank" class="nav-link nav-link-lg" data-toggle="tooltip" data-original-title="{{ __('POS') }}"><i class="bx bx-printer"></i></a>
            </li>
        @endif
        @if(addon_is_activated('pos_system') && settingHelper('is_pos_activated_for_seller') && authUser()->user_type == 'seller')
            <li>
                <a  href="{{ route('seller.pos.system') }}" target="_blank" class="nav-link nav-link-lg" data-toggle="tooltip" data-original-title="{{ __('POS') }}"><i class="bx bx-printer"></i></a>
            </li>
        @endif
        <li>
            <a href="{{ Sentinel::getUser()->user_type == 'seller' ? config('app.url').'/shop/'.Sentinel::getUser()->sellerProfile->slug : url('/') }}" target="_blank" class="nav-link nav-link-lg" data-toggle="tooltip" data-original-title="{{ __('Visit Store') }}"><i class="bx bx-globe"></i></a>
        </li>
        <li class="dropdown dropdown-list-toggle"><a href="#" data-toggle="dropdown" class="nav-link notification-toggle nav-link-lg {{ $notificationCount > 0 ? 'beep' : '' }} "><i class="bx bx-bell"></i></a>
            <div class="dropdown-menu dropdown-list dropdown-menu-right">
                ...
                <div class="dropdown-list-content dropdown-list-icons">
                    ...
                    <a href="{{ $notification->url != '' || $notification->url != null ? url($user_type.'/'.$notification->url) : "javascript:void(0)" }}" class="dropdown-item dropdown-item-unread">
        ...
        <li class="dropdown">
            ...
            <div class="dropdown-menu dropdown-menu-right">
                ...
                <a rel="alternate" hreflang="{{ $active_lang->locale }}" class="dropdown-item has-icon {{ App::getLocale() == $active_lang->locale ? 'active' : ''}}"
                   href="{{ LaravelLocalization::getLocalizedURL($active_lang->locale, null, [], true) }}">
                    <img alt="{{ $active_lang->name }}" src="{{ static_asset($active_lang->flag) }}" class="language-flag">
                    {{ $active_lang->name }}
                </a>
                @endforeach
            </div>
        ...
        <li class="dropdown"><a href="#" data-toggle="dropdown"
                ...
                @if(Sentinel::getUser()->images && array_key_exists('image_40x40',Sentinel::getUser()->images) && @is_file_exists(Sentinel::getUser()->images['image_40x40']))
                    ...
                    <img alt="{{Sentinel::getUser()->first_name}}" src="{{static_asset(Sentinel::getUser()->images['image_40x40'])}}" class="rounded-circle mr-1">
                @else
                    <img alt="{{Sentinel::getUser()->first_name}}" src="{{static_asset('images/default/user32x32.jpg')}}" class="rounded-circle mr-1">
                @endif
            ...
            <div class="dropdown-menu dropdown-menu-right">
                ...
                @endif
                <a href="{{ Sentinel::getUser()->user_type == 'seller' ? route('seller.profile') : route('admin.profile') }}" class="dropdown-item has-icon">
                    <i class="bx bx-user"></i> {{ __('Profile') }}
                </a>
                <a href="{{ Sentinel::getUser()->user_type == 'seller' ? route('seller.login.activity') : route('admin.login.activity') }}" class="dropdown-item has-icon">
