...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  methods: {
    ...
    parseData: function parseData(carts, checkouts, coupons) {
      ...
      if (this.authUser) {
        ...
        for (var _i = 0; _i < carts.length; _i++) {
          this.carts.push(carts[_i]);
          this.payment_form.quantity.push({
            id: carts[_i].id,
            quantity: carts[_i].quantity
          });
          this.payment_form.sub_total += carts[_i].price * carts[_i].quantity;
          this.payment_form.discount_offer += parseFloat(carts[_i].discount * carts[_i].quantity);

          if (this.settings.shipping_cost == 'product_base') {
            this.payment_form.shipping_tax += carts[_i].shipping_cost;
          }

          this.payment_form.tax += parseFloat(carts[_i].tax * carts[_i].quantity);
        }
      }
      ...
      if (coupons && this.settings.coupon_system == 1) {
        ...
        for (var _i2 = 0; _i2 < coupons.length; _i2++) {
          this.payment_form.coupon_discount += parseFloat(coupons[_i2].discount);
        }
      }
    ...
    activeSeller: function activeSeller(seller_id) {
      if (this.active_seller == seller_id) {
        this.active_seller = 0;
      } else {
        this.active_seller = seller_id;
      }
    }
  }
});
                            ...
                            _c("ul", { staticClass: "global-list" }, [
                              ...
                              _c("li", [
                                ...
                                _c("span", [
                                  _vm._v(
                                    _vm._s(
                                      _vm.priceFormat(_vm.payment_form.tax)
                                    )
                                  ),
                                ]),
                              ]),
                            ...
                            _c("div", { staticClass: "order-total" }, [
                              _c("p", [
                                ...
                                _c("span", [
                                  _vm._v(
                                    _vm._s(
                                      _vm.priceFormat(_vm.payment_form.total)
                                    )
                                  ),
                                ]),
                              ]),
