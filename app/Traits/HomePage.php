...
trait HomePage
{
    ...
    public function parseSettingsData($media, $category, $seller, $brand, $campaign,$shopping): ?array
    {
            ...
            foreach ($settings as $key => $setting) {
                foreach ($setting as $set_key => $item) {
                    ...
                    if ($set_key == 'download_section') {
                        $image = $media->get($item['banner']);
                        if ($image)
                            $item['banner'] = $image && is_file_exists($image->image_variants['image_320x320'], $image->image_variants['storage']) ? @get_media($image->image_variants['image_320x320'], $image->image_variants['storage']) : static_asset('images/default/default-image-320x320.png');
                        else {
                            $item['banner'] = static_asset('images/default/default-image-320x320.png');
                        }
                        $item['play_store'] = settingHelper('play_store_link');
                        $item['apple_store'] = settingHelper('apple_store_link');

                        $results = $this->keyDefine('download_section', $key, $item, $results);
