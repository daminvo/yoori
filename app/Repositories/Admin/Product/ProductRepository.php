...
class ProductRepository implements ProductInterface
{
    ...
    {
        $products = Product::with('createdBy', 'productLanguages','user')
            ->where('is_deleted', 0)
            ->CheckSellerSystem()
        ...
        try {
            ...
            if ($request->has('shipping_type')) :
                ...
                elseif ($request->shipping_type == 'flat_rate'):
                    $product->shipping_fee = $request->shipping_fee != '' ? $request->shipping_fee : 0.00;
                endif;
            ...
            if ($request->campaign) :
                ...
                $campaign_product->discount         = $request->campaign_discount;
                ...
                $product->special_discount          = $request->campaign_discount;
...
//            $product->created_by                = Sentinel::getUser()->id;
    ...
    public function productView($request)
    {
        ...
        try {
            ...
            $user_id                = authId();
            $view                   = ProductView::where('product_id', $product->id)->first();
            if ($view == null):
    ...
    public function viewedProduct()
    {
        ...
        })->selectRaw('id,thumbnail,slug,price,rating,current_stock,reward,minimum_order_quantity,special_discount,special_discount_start,special_discount_end')->ProductPublished()->UserCheck()->IsWholesale()->IsStockOut()->latest()->take(10)->get();
        ...
        if (count($viewed_products) < 20) {
            ...
            })->selectRaw('id,thumbnail,slug,price,rating,current_stock,reward,minimum_order_quantity,special_discount,special_discount_start,special_discount_end')->ProductPublished()->whereNotIn('id', $viewed_products->pluck('id')->toArray())->UserCheck()->IsWholesale()->IsStockOut()->latest()->take($left_product)->get();
    ...
    {
        return Product::withAvg('reviews','rating')->ProductPublished()->UserCheck()->IsWholesale()->IsStockOut()
    ...
    {
        $category = Category::where('slug', $slug)->first();
        return Product::withAvg('reviews','rating')->where('category_id', $category->id)->ProductPublished()->UserCheck()->IsWholesale()->IsStockOut()->paginate(8);
    }
    ...
    {
        return Product::with('userWishlist')->withAvg('reviews','rating')->withCount('reviews')->where('category_id', $id)->where('id', '!=', $product_id)->ProductPublished()
            ->UserCheck()->IsWholesale()->IsStockOut()->take($take)->get();
    }

    public function compareList(): int
    {
        if (authUser()) {
            $count = CompareProduct::where('user_id', authId())->count();
        } else {
    ...
    public function productByIds($ids)
    {
        return Product::with('brand:id', 'category:id','userWishlist')->withAvg('reviews','rating')->withCount('reviews')
            ...
            ->whereIn('id', $ids)->UserCheck()->IsWholesale()->IsStockOut()->get();
    }
    ...
    public function searchProduct($key)
    {
        ...
        })->selectRaw('id,brand_id,category_id,status,price,special_discount,special_discount_type,special_discount_start,special_discount_end,rating,slug,thumbnail,minimum_order_quantity,has_variant,reward,current_stock')->ProductPublished()
            ->UserCheck()->IsWholesale()->IsStockOut()->latest()->take(20)->get();
    }
    //api end
