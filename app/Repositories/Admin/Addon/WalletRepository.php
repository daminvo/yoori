...
class WalletRepository implements WalletInterface
{
    ...
    public function walletRecharge($data)
    {
        ...
        if (array_key_exists('id', $data))
        {
            ...
            if (array_key_exists('payment_type',$data) && $data['payment_type'] != 'ssl_commerze')
            {
                $payment_details = $this->methodCheck($data,[]);
            }
        }

        if (array_key_exists('user_id',$data) && array_key_exists('transaction_id',$data))
        {
            $data['payment_type'] = 'ssl_commerze';
