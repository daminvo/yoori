...
class SettingRepository implements SettingInterface{
    ...
    public function update($request)
    {
        ...
        try {
                    ...
                    if ($request->invoice_based_shipping_fee)
                    {
                        ...
                        foreach ($request->invoice_based_shipping_fee as $invoice_key=> $item) {
                            if ($invoice_key == 'max_amount')
                            {
                                ...
                                {
                                    $invoice_array[$k]['max_amount'] = $data;
                                }
                            }
                            if ($invoice_key == 'min_amount')
                            {
                                foreach ($item as $k=> $data)
                                {
                                    $invoice_array[$k]['min_amount'] = $data;
                                }
                            }
                            if ($invoice_key == 'fee')
                            {
            ...
            if($request->has('google_client_id') && checkEmptyProvider('is_google_login_activated')):
                envWrite('GOOGLE_CLIENT_ID', settingHelper('google_client_id'));
                envWrite('GOOGLE_CLIENT_SECRET', settingHelper('google_client_secret'));
                envWrite('GOOGLE_CALLBACK_URL', url('login/google/callback'));
            endif;
            if($request->has('facebook_client_id') && checkEmptyProvider('is_facebook_login_activated')):
                envWrite('FACEBOOK_CLIENT_ID', settingHelper('facebook_client_id'));
                envWrite('FACEBOOK_CLIENT_SECRET', settingHelper('facebook_client_secret'));
                envWrite('FACEBOOK_CALLBACK_URL', url('login/facebook/callback'));
            endif;
            if($request->has('twitter_client_id') && checkEmptyProvider('is_twitter_login_activated')):
                envWrite('TWITTER_CLIENT_ID', settingHelper('twitter_client_id'));
                envWrite('TWITTER_CLIENT_SECRET', settingHelper('twitter_client_secret'));
                envWrite('TWITTER_CALLBACK_URL', url('login/twitter/callback'));
            endif;
