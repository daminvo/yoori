    ...
    public function store($request)
    {
        ...
        try {
            ...
            foreach ($ids as $key => $value):
                ...
                $campaignProduct->discount      = $request['discount_'.$value];
...
//        dd($request->all());
    ...
    {
        return Campaign::with('currentLanguage')->whereIn('id',$ids)->select('id','banner','slug','thumbnail','start_date','end_date')->where('start_date','<=',now())->where('end_date','>=',now())->where('status',1)->latest()->take(3)->get();
    }
