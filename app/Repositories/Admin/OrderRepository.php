...
{
    use RandomStringTrait, PaymentTrait;
    ...
    {
        return Order::when(settingHelper('seller_system') != 1, function ($q) {
            $q->where('seller_id',1);
        })->find($id);
    }
    ...
    public function digitalProductOrders($limit)
    {
        if (authUser()) {
            return OrderDetail::with('product:id,thumbnail,slug,product_file_id', 'order')
                ->whereHas('order', function ($query) {
    ...
    public function productOrderList($item,$user)
    {
        if ($user) {
            ...
            },'order:id,date,user_id,is_deleted,status,payment_type,seller_id,code,delivery_status,total_payable'])
    ...
    public function confirmOrder($checkout, $carts, $data,$user)
    {
        ...
        if (count($carts) > 0)
        {
            foreach ($checkout as $key => $cart_group) {
                ...
                elseif (settingHelper('shipping_fee_type') == 'area_base' && $shipping) {
                ...
                foreach ($carts->whereIn('seller_id', $cart_group['seller_id']) as $item) {
                    ...
                    if ($item->shipping_cost > 0 && settingHelper('shipping_fee_type') != 'invoice_base') {
                        ...
                        $shipping_cost = [
                            ...
                            'total_cost'         => $shipping ? $item->shipping_cost : 0
                        ];
                    }
    ...
    public function completeOrder($data, $user,$offline)
    {
        ...
        if ($user || (array_key_exists('guest', $data) && $data['guest'] == 1))
        {
            ...
            if ($data['payment_type'] == 'mollie')
            {
                ...
                {
                    return back()->with(['error' => __('choose_paid_in_mollie')]);
                }
            }
            ...
            session()->put('trx_id',$orders->first()->trx_id);
        }
    ...
    public function sslResponse($data,$user)
    {
        $orders = [];
        if (array_key_exists('code', $data) && $data['code'] && $data['code'] != 'undefined') {
            $orders = $this->orderByCodes($data['code']);
        } else {
            if (array_key_exists('trx_id',$data) && !empty($data['trx_id']))
            {
                $orders = $this->takePaymentOrder($data['trx_id']);
            }
        }

        return $this->sslCommerze($data, $orders,$user);
    }
