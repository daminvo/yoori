...
class OrderController extends Controller
{
    ...
    public function profileOrder(OrderInterface $order): \Illuminate\Http\JsonResponse
    {
        try {
            $data = [
        ...
        try {
            ...
            if ($order):
                ...
                try {
                    ...
                    if ($data['order'] && authUser() && $data['order']->user_id != authId())
                    {
                        return response()->json([
                            'error' => __('Not found')
                        ]);
                    }
                    if ($data['order'] && !authUser() && $data['order']->user_id != getWalkInCustomer()->id)
                    {
                        return response()->json([
                            'error' => __('Not found')
                        ]);
    ...
    public function userLastOrder(CartInterface $cart, OrderInterface $order, CurrencyInterface $currency, OfflineMethodInterface $offlineMethod, Request $request): \Illuminate\Http\JsonResponse
    {
        try {
            if (!empty($request->code)) {
                ...
                $check_code = $order->checkCodByTrx($carts->first()->trx_id);
            }
    ...
    public function completeOrder(OrderInterface $order, OfflineMethodInterface $offlineMethod, Request $request)
    {
        ...
        try {
            $data = [
                'orders' => $order->completeOrder($request->all(), authUser(), $offlineMethod),
    ...
    public function sendMail(OrderInterface $order, $trx_id): \Illuminate\Http\JsonResponse
    {
        DB::beginTransaction();
        try {
            $orders = $order->invoiceByTrx($trx_id);
