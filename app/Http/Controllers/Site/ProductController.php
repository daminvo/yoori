...
class ProductController extends Controller
{
    ...
    public function productDetails($slug, ColorInterface $color, AttributeInterface $attribute): \Illuminate\Http\JsonResponse
    {
        try {
            ...
            $images = $original_images          = $wholesale_prices = $thumbnails = [];
            ...
            if($product->images)
            {
                foreach ($product->images as $image) {
                    ...
                    {
                        $images[$image_no]      = @get_media(@$image['image_320x320'], @$image['storage']);
                        $thumbnails[$image_no]  = @get_media(@$image['image_72x72'], @$image['storage']);

                        $original_images[$image_no] = [
                            'id'     =>   $image_no,
                            'url'    =>   @get_media(@$image['original_image'], @$image['storage'])
                        ];
            ...
            if ($product->has_variant) {
                foreach ($stock as $key => $item) {
                    ...
                    {
                        $images[$image_no]      = @get_media(@$item->image['image_320x320'], @$item->image['storage']);

                        $thumbnails[$image_no]  = @get_media(@$item->image['image_72x72'], @$item->image['storage']);

                        $original_images[$image_no] = [
                            'id'     =>   $image_no,
                            'url'    =>   @get_media(@$item->image['original_image'], @$item->image['storage'])
                        ];
            ...
            $images[$image_no]      = @getFileLink('320x320', @$product->thumbnail);
            $thumbnails[$image_no]  = @getFileLink('72x72', @$product->thumbnail);

            $original_images[$image_no] = [
                'id'     =>   $image_no,
                'url'    =>   @get_media(@$product->thumbnail['original_image'], @$product->thumbnail['storage'])
            ];



            $product->makeHidden(['images', 'image_ids', 'brand_id', 'thumbnail', 'thumbnail_id', 'meta_image', 'meta_image_id',
...
//                $products = ProductResource::collection($this->product->adminProducts());
            ...
            $product->form                      = [
                ...
                'variants_ids'      => @$first_stock->variant_ids,
                'variants_name'     => @$first_stock->name,
            ];
    ...
    public function productView(Request $request, ProductInterface $product)
    {
        try {
            $data = [
                ...
                'viewProduct' => $product->viewedProduct(),
            ];
            return response()->json($data);
        } catch (\Exception $e) {
            return response()->json([
