...
class EarningController extends Controller
{
    ...
    public function myEarnings(Request $request): \Illuminate\Http\JsonResponse
    {
        try {
            ...
            $data['today_earning']                  = get_price($this->deliveryHero->getCommission($delivery_hero_id, 'today'));
            $data['yesterday_earning']              = get_price($this->deliveryHero->getCommission($delivery_hero_id, 'yesterday'));
            $data['lifetime_earning']               = get_price($user->deliveryHero->total_commission);
            ...
            foreach ($transaction_history as $transaction):
                $formated_transaction_history[] = [
                    ...
                    'created_at'    => Carbon::parse($transaction->created_at)->format('d F Y'),
                ];
            endforeach;
